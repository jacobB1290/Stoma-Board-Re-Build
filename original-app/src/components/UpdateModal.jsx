...
export default function UpdateModal({ open, onClose }) {
  ...
  const { outdatedUsers, currentUsers, userStats } = useMemo(() => {
    ...
    let filtered = usersData.filter((user) => {
      ...
      const testPatterns = /^(test|asdf|qwer|xxx|yyy|zzz|\d+)$/i;
      return !testPatterns.test(name);
    });

    const grouped = {};
    filtered.forEach((user) => {
      const key = (user.user_name || "").trim().toLowerCase();
      if (
        !grouped[key] ||
        new Date(user.last_seen) > new Date(grouped[key].last_seen)
      ) {
        grouped[key] = user;
      }
    });

    const allUsers = Object.entries(grouped).map(([key, user]) => ({
      ...user,
      key,
    ...
    // Sort function: active/idle alphabetically, offline by most recent first
    const sortFn = (a, b) => {
      ...
      // For offline users, sort by most recent (latest) first
      if (a.status === "offline") {
        return new Date(b.last_seen) - new Date(a.last_seen);
      }

      // For active/idle, sort alphabetically for stability
