...
export const addCase = async ({
  ...
  // Build modifiers array including stage for digital cases
  const modifiers = [
    ...(rush ? ["rush"] : []),
    ...(hold ? ["hold"] : []),
    ...(caseType === "bbs" ? ["bbs"] : []),
    ...(caseType === "flex" ? ["flex"] : []),
    // If it's a Digital case needing repair, start in finishing
    // Otherwise, start in design
    ...(department === "Digital"
...
export const updateCase = async (payload) => {
  ...
  const existingStageModifiers =
    prev.modifiers?.filter(
      (m) => m.startsWith("stage-") || m.startsWith("stats-exclude")
    ) || [];
  ...
  ["rush", "hold", "bbs", "flex"].forEach((f) => {
    const d = diff(f);
    if (d.was !== d.now) logs.push(d.now ? `${f} added` : `${f} removed`);
  });
...
export const toggleCaseExclusion = async (
  ...
  // Check if case is currently excluded
  ...
  if (isCurrentlyExcluded) {
    // Include the case - just use filtered modifiers
    action = stage
